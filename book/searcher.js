...
(function search() {
	...
	function renderURL(urlobject) {
		...
		for (const prop in urlobject.params) {
			if (Object.hasOwn(urlobject.params, prop)) {
	...
	function formatSearchResult(result, searchterms) {
		...
		// representation (%27).
		const encoded_search = encodeURIComponent(searchterms.join(" ")).replace(
			/'/g,
			"%27",
		);
	...
	function doSearchOrMarkFromUrl() {
		...
		if (
			Object.hasOwn(url.params, URL_SEARCH_PARAM) &&
		...
		if (Object.hasOwn(url.params, URL_MARK_PARAM)) {
	...
	function globalKeyHandler(e) {
		...
		if (e.key === "Escape") {
			...
			window.scrollTo(0, 0);
			searchbar.select();
		} else if (hasFocus() && (e.key === "ArrowDown" || e.key === "Enter")) {
			e.preventDefault();
	...
	function setSearchUrlParameters(searchterm, action) {
		const url = parseURL(window.location.href);
		const first_search = !Object.hasOwn(url.params, URL_SEARCH_PARAM);
