...
# shellcheck disable=SC2016  # GraphQL variables like $login/$number are not shell expansions
...
if [[ -z ${STATUS_FIELD_ID} ]]; then
  ...
  # shellcheck disable=SC2016  # GraphQL variables like $projectId are not shell expansions
...
for opt in "${REQUIRED_OPTIONS[@]}"; do
  if ! echo "${CURRENT_OPTIONS}" | grep -Fxq "${opt}"; then
    ...
    # shellcheck disable=SC2016  # GraphQL variables: $projectId, $fieldId, $name
